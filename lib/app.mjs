...
async function adminAnnouncements(req, res, u) {
  ...
  if (req.method === 'PATCH') {
    ...
    const fields = [];
    const vals = [];
    if (input.title !== undefined) { fields.push(`title=$${vals.length+1}`); vals.push(String(input.title).slice(0,120)); }
    if (input.body !== undefined) { fields.push(`body=$${vals.length+1}`); vals.push(String(input.body).slice(0,2000)); }
    if (input.priority !== undefined && ['low','normal','high','urgent'].includes(input.priority)) { fields.push(`priority=$${vals.length+1}`); vals.push(input.priority); }
