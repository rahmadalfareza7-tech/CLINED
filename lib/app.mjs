...
let chatTablesPromise = null;
async function ensureChatTables(){
  if (chatTablesPromise) return chatTablesPromise;
  chatTablesPromise = (async () => {
  const db=getPool();
...
async function aiExplain(req,res){
  ...
  origin(req);
  const u=await required(req,res); if(!u)return;
  if(!limit(req,res,20))return; // 20 pertanyaan AI / menit / user
  const key=String(process.env.OPENAI_API_KEY||'').trim();
  if(!key)return fail(res,503,'Fitur AI belum diaktifkan (OPENAI_API_KEY belum diatur).');
  const x=await body(req);
  const soal=String(x.soal||'').slice(0,2000);
  const opsi=Array.isArray(x.opsi)?x.opsi.slice(0,6).map(o=>String(o).slice(0,300)):[];
  const jawabanBenar=String(x.jawabanBenar||'').slice(0,300);
  const jawabanUser=String(x.jawabanUser||'').slice(0,300);
  const pertanyaan=String(x.pertanyaan||'').slice(0,500);
  if(!soal)return fail(res,400,'Soal wajib diisi.');
  const sys='Kamu adalah tutor kedokteran untuk mahasiswa profesi. Jawab dalam Bahasa Indonesia, ringkas (maks 150 kata), jelaskan alasan klinis/fisiologis di balik jawaban, jangan mengulang soal secara verbatim.';
  const userMsg=`Soal: ${soal}\nOpsi: ${opsi.join(' | ')}\nJawaban benar: ${jawabanBenar}\nJawaban mahasiswa: ${jawabanUser||'(belum jawab)'}\nPertanyaan mahasiswa: ${pertanyaan||'Tolong jelaskan kenapa jawaban itu benar dan yang lain salah.'}`;
  let resp;
  try{
    resp=await fetch('https://api.openai.com/v1/chat/completions',{
      method:'POST',
      headers:{'Content-Type':'application/json','Authorization':`Bearer ${key}`},
      body:JSON.stringify({model:OPENAI_MODEL,temperature:0.3,max_tokens:400,messages:[{role:'system',content:sys},{role:'user',content:userMsg}]})
    });
  }catch(e){return fail(res,502,'Gagal menghubungi layanan AI.');}
  if(!resp.ok){const t=await resp.text().catch(()=>'');return fail(res,502,`Layanan AI menolak permintaan (${resp.status}). ${t.slice(0,200)}`);}
  const data=await resp.json().catch(()=>null);
  const answer=data?.choices?.[0]?.message?.content?.trim();
  if(!answer)return fail(res,502,'Layanan AI tidak mengembalikan jawaban.');
...
export async function handleRequest(req,res){
  ...
  try{
    ...
    if(path==='/api/ai/explain')return await aiExplain(req,res);
    if(path.startsWith('/api/photos'))return await photos(req,res,path.slice(11));
    if(path==='/api/banks'||path.startsWith('/api/banks/'))return await api(req,res,path.slice(4));if(path==='/api/materials')return await publicMaterials(req,res);if(path==='/api/admin/materials'||path.startsWith('/api/admin/materials/'))return await adminMaterials(req,res,await user(req));if(path.startsWith('/api/content/'))return await contentApi({request:req,response:res,url,user:await user(req),pool:getPool(),body,json,fail,origin});
