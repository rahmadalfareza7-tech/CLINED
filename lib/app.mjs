...
async function adminUsers(req,res,u){
  ...
  if(deleteMatch){
    ...
    try{
      await client.query('BEGIN');
      // Konten publik yang dibuat user tetap dipertahankan dan kepemilikannya dipindahkan ke admin yang menghapus.
      await client.query('UPDATE content_packages SET created_by=$1 WHERE created_by=$2',[u.id,targetId]);
