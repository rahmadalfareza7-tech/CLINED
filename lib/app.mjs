...
async function adminMaterials(req,res,u){
  ...
  // Use collection POST actions instead of dynamic /:block routes.
  // This is more reliable on Vercel/serverless deployments and avoids 404s
  // caused by catch-all routing of encoded block names.
  if(req.method==='POST'){
    origin(req);
    const input=await body(req);
    const action=String(input.action||'save').toLowerCase();
    const block=String(input.block||'').trim().slice(0,80);
    if(!block) return fail(res,400,'Blok materi wajib dipilih.');
    if(action==='delete'){
    ...
    if(action!=='save') return fail(res,400,'Aksi materi tidak valid.');
