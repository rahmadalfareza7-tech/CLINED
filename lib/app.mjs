...
async function adminActivity(req,res,u){
  ...
  const r=await getPool().query(`
    ...
    LIMIT 200`);
  return json(res,200,{activities:r.rows.map(x=>({
...
async function adminUsers(req,res,u){
  ...
  if(req.method==='GET' && pathname==='/api/admin/users'){
    ...
    return json(res,200,{users:r.rows.map(publicUser)});
  }
  ...
  const r=await getPool().query('UPDATE users SET role=$1,updated_at=now() WHERE id=$2 RETURNING id,name,username,role,created_at,last_login_at',[role,targetId]);
  await getPool().query('INSERT INTO activity_logs (id,user_id,action,metadata) VALUES ($1,$2,$3,$4::jsonb)',[
