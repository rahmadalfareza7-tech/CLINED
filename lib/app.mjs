...
async function api(req,res,path){if(path==='/health'&&req.method==='GET'){await getPool().query('SELECT 1');return json(res,200,{ok:true});}if(path==='/banks'&&req.method==='GET'){const r=await getPool().query('SELECT id,name,block,version,schema_version,jsonb_array_length(questions) AS count,updated_at FROM question_banks WHERE published=true ORDER BY id');return json(res,200,{banks:r.rows});}const match=path.match(/^\/banks\/([a-z0-9_-]+)$/i);if(match&&req.method==='GET'){const r=await getPool().query('SELECT id,name,block,version,schema_version,questions,updated_at FROM question_banks WHERE id=$1 AND published=true',[match[1]]);if(!r.rowCount)return fail(res,404,'Bank soal tidak ditemukan.');const b=r.rows[0];const etag=`"${b.id}-v${b.version}"`;if(req.headers['if-none-match']===etag){res.writeHead(304,{'ETag':etag,'Cache-Control':'public, max-age=3600, stale-while-revalidate=86400'});return res.end();}res.writeHead(200,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'public, max-age=3600, stale-while-revalidate=86400','ETag':etag});return res.end(JSON.stringify({bank:b}));}return fail(res,404,'Endpoint tidak ditemukan.');}
...
async function leaderboard(req, res) {
  ...
  const r = await getPool().query(
    `SELECT u.id, u.name, u.username,
     ...
     ORDER BY COALESCE(s.xp_total,0) DESC, u.name ASC LIMIT 50`,
