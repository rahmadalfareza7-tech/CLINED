...
async function chat(req,res,path){
  const u=await required(req,res); if(!u)return;
  origin(req);
  // User directory does not depend on chat tables. Keeping this endpoint independent
  // prevents a chat-table migration/creation problem from hiding the account list.
...
export async function handleRequest(req,res){
  ...
  try{
    ...
    if(path.startsWith('/api/chat/'))return await chat(req,res,path.slice(9));
