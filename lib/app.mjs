...
function isDeveloper(u){return u&&u.role==='developer';}
function isAdmin(u){return u&&(u.role==='admin'||u.role==='developer');}
async function required(req,res,admin=false){const u=await user(req);if(!u){fail(res,401,'Silakan masuk terlebih dahulu.');return null;}if(admin&&!isAdmin(u)){fail(res,403,'Akses tidak diizinkan.');return null;}return u;}
...
async function ensureAdmin(){
  try{
    ...
    if(existing.rowCount && existing.rows[0].role==='developer' && !resetPassword)return;
    const pw=await hash(password),id=randomUUID();
    await getPool().query(`INSERT INTO users (id,name,username,email,password_hash,password_salt,role) VALUES ($1,$2,$3,NULL,$4,$5,'developer')
      ON CONFLICT (username) DO UPDATE SET name=EXCLUDED.name,role='developer',password_hash=EXCLUDED.password_hash,password_salt=EXCLUDED.password_salt,updated_at=now()`,[id,name,username,pw.hash,pw.salt]);
...
async function adminBanks(req,res,path,u){
  if (!u || !isAdmin(u)) return fail(res,403,'Akses admin diperlukan.');
...
async function adminLogins(req,res,u){
  if (!u || !isDeveloper(u)) return fail(res,403,'Akses developer diperlukan.');
...
async function adminUsers(req,res,u){
  ...
  if(req.method==='GET' && pathname==='/api/admin/users'){
    const r=await getPool().query('SELECT id,name,username,role,created_at,last_login_at FROM users ORDER BY CASE WHEN role=\'developer\' THEN 0 WHEN role=\'admin\' THEN 1 ELSE 2 END,name');
  ...
  if(!['student','admin','developer'].includes(role)) return fail(res,400,'Role tidak valid.');
  if(targetId===u.id && role!=='developer') return fail(res,400,'Kamu tidak dapat menurunkan role akun developer yang sedang dipakai.');
  const target=(await getPool().query('SELECT id,name,username,role FROM users WHERE id=$1',[targetId])).rows[0];
  if(!target) return fail(res,404,'Pengguna tidak ditemukan.');
