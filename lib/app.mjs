...
async function sync(req,res){
  ...
  try{
    ...
    for(const op of ops.slice(0,SYNC_MAX_KEYS)){
      ...
      // XP is stored in the account sync state; mirror it into the indexed weekly
      // leaderboard snapshot in the same authenticated request.
      if(op.type==='set' && key==='gaster_v11_xp'){
        let xpValue=0;
        try{xpValue=Number(JSON.parse(op.value));}catch{xpValue=Number(op.value);}
        await upsertXpSnapshot(u.id, xpValue);
      }
    }
