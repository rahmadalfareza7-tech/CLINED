...
const validKey=k=>/^(gaster_|clined_|medicalRpg|medical_rpg|current_block|last_worked|dashboard_block|alpha5_)/.test(k)&&!/auth|password|session|token|sync_queue/i.test(k);
...
async function leaderboard(req, res) {
  ...
  const r = await getPool().query(
    `SELECT u.id, u.name, u.username,
       COALESCE(s.xp_total, 0) AS xp_weekly,
       x.state->>'gaster_v11_xp' AS xp_all_time
     FROM users u
