...
(() => {
  ...
  function install(user){
    if(!user || (user.role!=='admin' && user.role!=='developer')){ removeAdminControls(); return; }
    ...
    if(!document.getElementById('adminContentWorkspace')){
      ...
      const kicker=isDeveloper?'DEVELOPER • CONTENT MANAGER':'ADMIN • CONTENT MANAGER';
      const badge=isDeveloper?'DEVELOPER':'ADMIN';
      box.innerHTML=`
        <div class="account-auth-top"><div><span class="account-auth-kicker">${kicker}</span><h3>Kelola Konten</h3><p>UAB dan UPI dipisahkan supaya alur input sesuai jenis kontennya.</p></div><span class="account-security-badge secure">${badge}</span></div>
        ...
        <div data-content-panel="upi" hidden>
          <div class="admin-content-intro"><b>Create Question UPI</b><span>Pilih blok, materi, gambar, pertanyaan, jawaban, lalu publikasikan ke semua pengguna.</span></div>
        ...
        ${isDeveloper?`<div class="admin-materials-section">
    ...
    if(isDeveloper && !document.getElementById('adminLoginsPanel')){
      const logins=document.createElement('section'); logins.id='adminLoginsPanel'; logins.className='account-auth-card';
      logins.innerHTML=`<div class="account-auth-top"><div><span class="account-auth-kicker">DEVELOPER • AKTIVITAS & PENGGUNA</span><h3>Aktivitas akun & role</h3><p>Riwayat login/logout dan perubahan role pengguna.</p></div><span class="account-security-badge secure" data-activity-count>0</span></div><div class="admin-panel-tabs" role="tablist"><button type="button" class="admin-panel-tab selected" data-admin-tab="activity">Aktivitas</button><button type="button" class="admin-panel-tab" data-admin-tab="users">Pengguna <span class="admin-tab-count" data-user-count>0</span></button><button type="button" class="admin-panel-tab" data-admin-tab="announcements">📢 Pengumuman</button></div><div data-admin-panel="activity"><div class="admin-activity-toolbar"><button type="button" class="admin-delete-btn danger clined-compact-action" data-reset-activity>Reset aktivitas login/logout</button></div><div data-activity-list class="muted">Memuat…</div></div><p class="muted" data-content-message></p><div data-admin-panel="announcements" hidden><div class="admin-announcements-section" style="padding:12px 0"><div class="admin-new-announce-form"><label style="display:block;font-size:11px;font-weight:900;text-transform:uppercase;letter-spacing:.05em;color:#6b777b;margin:0 0 4px">Judul</label><input type="text" id="annTitleInput" maxlength="120" placeholder="Judul pengumuman…" style="width:100%;padding:10px 12px;border:2px solid var(--line);border-radius:12px;background:var(--surface);color:var(--text);font:inherit;font-size:13px;margin-bottom:8px"><label style="display:block;font-size:11px;font-weight:900;text-transform:uppercase;letter-spacing:.05em;color:#6b777b;margin:0 0 4px">Isi Pengumuman</label><textarea id="annBodyInput" rows="3" maxlength="2000" placeholder="Tulis isi pengumuman…" style="width:100%;padding:10px 12px;border:2px solid var(--line);border-radius:12px;background:var(--surface);color:var(--text);font:inherit;font-size:13px;resize:vertical;margin-bottom:8px"></textarea><div style="margin-bottom:10px"><select id="annPrioritySelect" style="padding:8px 10px;border:2px solid var(--line);border-radius:10px;background:var(--surface);color:var(--text);font:inherit"><option value="low">Low</option><option value="normal" selected>Normal</option><option value="high">High</option><option value="urgent">Urgent</option></select></div><button type="button" class="primary" id="saveAnnBtn" style="margin-bottom:12px">Simpan Pengumuman</button></div><div id="annListContainer"><div class="muted" style="padding:8px">Memuat pengumuman…</div></div></div></div><div data-admin-panel="users" hidden><div class="admin-users-scroll"><div data-users-list class="muted">Memuat…</div></div></div>`;
      ...
      const renderAnnouncements=async()=>{const c=logins.querySelector('#annListContainer');if(!c)return;try{const d=await request('/api/admin/announcements');const anns=d.announcements||[];c.innerHTML=anns.length?anns.map(a=>`<div style="padding:10px;border:1px solid var(--line);border-radius:12px;margin-bottom:8px;background:var(--surface)"><div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><span style="font-size:9px;font-weight:900;letter-spacing:.06em;padding:3px 7px;border-radius:999px;background:rgba(0,0,0,.07)">${esc(a.priority.toUpperCase())}</span><b style="font-size:13px">${esc(a.title)}</b><span style="margin-left:auto;font-size:10px;font-weight:800">${a.published?'Dipublikasi':'Draft'}</span></div><p style="font-size:12px;color:var(--muted);margin:0 0 8px">${esc(a.body)}</p><div style="display:flex;gap:8px"><button type="button" class="secondary small-btn" data-ann-toggle="${esc(a.id)}">${a.published?'Jadikan Draft':'Publikasikan'}</button><button type="button" class="secondary small-btn" style="color:#ef4444;border-color:rgba(239,68,68,.35)" data-ann-del="${esc(a.id)}">Hapus</button></div></div>`).join(''):'<p class="muted">Belum ada pengumuman.</p>';c.querySelectorAll('[data-ann-toggle]').forEach(btn=>btn.addEventListener('click',async()=>{const id=btn.dataset.annToggle;const pub=!anns.find(a=>a.id===id)?.published;btn.disabled=true;try{await request('/api/admin/announcements',{method:'POST',body:JSON.stringify({action:pub?'publish':'draft',id})});await renderAnnouncements();}catch(e){message(logins,e.message||'Aksi pengumuman gagal.',true);btn.disabled=false;}}));c.querySelectorAll('[data-ann-del]').forEach(btn=>btn.addEventListener('click',async()=>{if(!confirm('Hapus pengumuman ini?'))return;btn.disabled=true;try{await request('/api/admin/announcements',{method:'POST',body:JSON.stringify({action:'delete',id:btn.dataset.annDel})});await renderAnnouncements();}catch(e){message(logins,e.message||'Penghapusan gagal.',true);btn.disabled=false;}}));}catch(e){c.textContent='Gagal memuat: '+(e&&e.message||'error');};};
      logins.querySelector('#saveAnnBtn')?.addEventListener('click',async()=>{const title=logins.querySelector('#annTitleInput')?.value?.trim();const body=logins.querySelector('#annBodyInput')?.value?.trim();const priority=logins.querySelector('#annPrioritySelect')?.value||'normal';const published=false;if(!title||!body){alert('Judul dan isi harus diisi.');return;}try{await request('/api/admin/announcements',{method:'POST',body:JSON.stringify({title,body,priority,published})});logins.querySelector('#annTitleInput').value='';logins.querySelector('#annBodyInput').value='';renderAnnouncements();if(typeof showToast==='function')showToast('Pengumuman tersimpan.',false);}catch(e){alert(e.message||'Gagal menyimpan.');}});
      logins.querySelectorAll('[data-admin-tab]').forEach(tab=>tab.addEventListener('click',async()=>{logins.querySelectorAll('[data-admin-tab]').forEach(x=>x.classList.toggle('selected',x===tab));logins.querySelectorAll('[data-admin-panel]').forEach(x=>x.hidden=x.dataset.adminPanel!==tab.dataset.adminTab);if(tab.dataset.adminTab==='users')await renderUsers();if(tab.dataset.adminTab==='announcements')await renderAnnouncements();}));
  ...
  async function boot(){try{const d=await request('/api/auth/me');if(d.user){window.__CLINED_AUTH_USER__=d.user;install(d.user);await Promise.all([hydrateMaterials(),hydrateGlobalBanks(),hydrateUpi(),hydrateUab()]);}else removeAdminControls();}catch(e){removeAdminControls();}}
  window.CLINED_INSTALL_ADMIN=()=>{const u=window.__CLINED_AUTH_USER__;if(u)install(u);};
