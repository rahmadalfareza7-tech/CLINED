...
(() => {
  ...
  async function hydrateGlobalBanks(){try{const catalog=await request('/api/banks');const items=catalog.banks||[];// Register stubs so UI shows bank names/counts immediately
    for(const item of items){const existing=window.CLINED_BANK_ENGINE?.getBank?.(item.id);if(existing&&Number(existing.version)>=Number(item.version))continue;// Only fetch if version changed or not cached
