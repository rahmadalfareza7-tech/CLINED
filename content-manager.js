...
(() => {
  ...
  function install(user){
    ...
    if(!document.getElementById('adminLoginsPanel')){
      ...
      const renderAnnouncements=async()=>{const c=logins.querySelector('#annListContainer');if(!c)return;try{const d=await request('/api/admin/announcements');const anns=d.announcements||[];c.innerHTML=anns.length?anns.map(a=>`<div style="padding:10px;border:1px solid var(--line);border-radius:12px;margin-bottom:8px;background:var(--surface)"><div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><span style="font-size:9px;font-weight:900;letter-spacing:.06em;padding:3px 7px;border-radius:999px;background:rgba(0,0,0,.07)">${esc(a.priority.toUpperCase())}</span><b style="font-size:13px">${esc(a.title)}</b><span style="margin-left:auto;font-size:10px;font-weight:800;color:${a.published?'#58cc02':'#6b777b'}">${a.published?'Dipublikasi':'Draft'}</span></div><p style="font-size:12px;color:var(--muted);margin:0 0 8px">${esc(a.body)}</p><div style="display:flex;gap:8px"><button type="button" class="secondary small-btn" data-ann-toggle="${esc(a.id)}" data-ann-pub="${a.published?'1':'0'}">${a.published?'Jadikan Draft':'Publikasikan'}</button><button type="button" class="secondary small-btn" style="color:#ef4444;border-color:rgba(239,68,68,.35)" data-ann-del="${esc(a.id)}">Hapus</button></div></div>`).join(''):'<p class="muted">Belum ada pengumuman.</p>';c.querySelectorAll('[data-ann-toggle]').forEach(btn=>btn.addEventListener('click',async()=>{const id=btn.dataset.annToggle,pub=btn.dataset.annPub!=='1';if(btn.disabled)return;btn.disabled=true;try{const d=await request('/api/admin/announcements',{method:'PATCH',body:JSON.stringify({id,published:pub})});if(typeof showToast==='function')showToast(d.message||(pub?'Pengumuman dipublikasikan.':'Pengumuman dijadikan draft.'),false);await renderAnnouncements();}catch(e){if(typeof showToast==='function')showToast(e.message||'Publikasi pengumuman gagal.',true);else alert(e.message||'Publikasi pengumuman gagal.');}finally{btn.disabled=false;}}));c.querySelectorAll('[data-ann-del]').forEach(btn=>btn.addEventListener('click',async()=>{if(!confirm('Hapus pengumuman ini?'))return;if(btn.disabled)return;btn.disabled=true;try{const d=await request('/api/admin/announcements',{method:'DELETE',body:JSON.stringify({id:btn.dataset.annDel})});if(typeof showToast==='function')showToast(d.message||'Pengumuman berhasil dihapus.',false);await renderAnnouncements();}catch(e){if(typeof showToast==='function')showToast(e.message||'Penghapusan pengumuman gagal.',true);else alert(e.message||'Penghapusan pengumuman gagal.');}finally{btn.disabled=false;}}));}catch(e){c.textContent='Gagal memuat: '+(e&&e.message||'error');};};
