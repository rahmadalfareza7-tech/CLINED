...
try {
  await pool.query(`INSERT INTO users (id,name,username,email,password_hash,password_salt,role) VALUES ($1,$2,$3,NULL,$4,$5,'developer')
    ON CONFLICT (username) DO UPDATE SET name=EXCLUDED.name,password_hash=EXCLUDED.password_hash,password_salt=EXCLUDED.password_salt,role='developer',updated_at=now()`, [randomUUID(), name.slice(0, 40), username, passwordHash, salt]);
  console.log(`Developer ${username} siap digunakan.`);
