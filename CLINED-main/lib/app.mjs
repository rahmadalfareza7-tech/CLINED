...
async function adminAnnouncements(req, res, u) {
  ...
  if (req.method === 'POST' && pathname === '/api/admin/announcements') {
    ...
    await getPool().query(
      ...
      [id, title, bodyText, priority, Boolean(input.published), u.id, expiresAt]
    );
    return json(res, 201, { id });
  }

  // Publish/unpublish dapat memakai endpoint koleksi agar tetap kompatibel
  // dengan deployment serverless yang tidak meneruskan dynamic route [id].
  const match = pathname.match(/^\/api\/admin\/announcements\/([0-9a-f-]{36})$/i);
  if (req.method === 'PATCH' && (pathname === '/api/admin/announcements' || match)) {
    origin(req);
    const input = await body(req);
    const aId = match ? match[1] : String(input.id || '').trim();
    if (!/^[0-9a-f-]{36}$/i.test(aId)) return fail(res, 400, 'ID pengumuman tidak valid.');
    const fields = [];
    const vals = [];
  ...
  if (req.method === 'DELETE') {
    origin(req);
    const input = await body(req);
    const deleteId = match ? match[1] : String(input.id || '').trim();
    if (!/^[0-9a-f-]{36}$/i.test(deleteId)) return fail(res, 400, 'ID pengumuman tidak valid.');
    const result = await getPool().query('DELETE FROM announcements WHERE id=$1', [deleteId]);
    if (!result.rowCount) return fail(res, 404, 'Pengumuman tidak ditemukan.');
    return json(res, 200, { ok: true, message: 'Pengumuman berhasil dihapus.' });
  }
