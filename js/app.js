...
const UPI_MATERIALS=[
  {id:'Histology',icon:'🔬',desc:'Gambaran mikroskopik jaringan'},
  {id:'Patologi Anatomi',icon:'🧬',desc:'Gambaran patologis jaringan dan organ'},
  {id:'Patologi Klinik',icon:'🧪',desc:'Pemeriksaan laboratorium dan interpretasi klinis'},
  {id:'Mikrobiologi',icon:'🦠',desc:'Bakteri, virus, jamur, dan mikroorganisme'},
  {id:'Fisiologi',icon:'⚙️',desc:'Fungsi normal tubuh dan mekanismenya'},
  {id:'Parasitologi',icon:'🪱',desc:'Parasit, siklus hidup, dan penyakit parasitik'}
];
...
function upiBuildMaterialPage(block,material){const id=upiMaterialPageId(block,material);let root=document.getElementById(id);if(!root){root=document.createElement('section');root.id=id;root.className='view exam-page-view upi-page upi-material-page';upiEnsureMaterialPagesHost().appendChild(root);}const b=UPI_BLOCKS.find(x=>x.id===block);const all=upiCardsForBlock(block);const cards=all.filter(c=>(c.material||'Histology')===material);const reviews=(()=>{const raw=get(UPI_REVIEW_PREFIX+block+'_'+material,{});return raw&&typeof raw==='object'&&!Array.isArray(raw)?raw:{};})();const due=cards.filter(c=>upiDue(c,reviews[c.id],Date.now())).length;root.innerHTML=`<div class="exam-page-header upi-header"><button class="back-link" type="button" data-upi-material-back>‹ ${esc(b.name)}</button><div><div class="eyebrow">UPI • ${esc(b.name.toUpperCase())} • ${esc(material.toUpperCase())}</div><h2>${esc(material)}</h2><p>Tebak gambar, ketuk kartu untuk buka jawaban, lalu nilai tingkat ingatanmu.</p></div></div><section class="upi-memory-overview card"><div class="upi-memory-head"><div><span class="upi-kicker">SPACED REPETITION</span><b>${esc(material)} • ${esc(b.name)}</b><small>Progres dan jadwal repetisi hanya berlaku untuk deck ini.</small></div><div class="upi-due-badge"><b data-upi-due>${due}</b><span>perlu diulang</span></div></div><div class="upi-memory-bars" data-upi-bars></div></section><section class="upi-flashcard card" aria-live="polite"><div class="upi-card-topline"><span data-upi-index>Kartu 1</span><span data-upi-topic>—</span></div><div class="upi-flip" data-upi-flip tabindex="0" role="button" aria-pressed="false"><div class="upi-flip-inner"><div class="upi-face upi-face-front"><div class="upi-image-frame"><img data-upi-image src="assets/flashcards/heart.svg" alt="Ilustrasi medis untuk ditebak"><span class="upi-image-hint">TEBAK DULU</span></div><div class="upi-prompt"><span class="eyebrow">PERTANYAAN</span><h3 data-upi-prompt>—</h3></div><div class="upi-flip-cta">Ketuk untuk buka jawaban <span>↗</span></div></div><div class="upi-face upi-face-back"><div class="upi-answer-label">JAWABAN</div><b data-upi-answer>—</b><p data-upi-explanation>—</p><div class="upi-flip-cta">Ketuk untuk kembali <span>↺</span></div></div></div></div><div class="upi-rating" data-upi-rating hidden><div class="upi-rating-title">Seberapa kuat kamu mengingatnya?</div><div class="upi-rating-grid"><div class="upi-rate again" data-rating="again" role="button" tabindex="0"><b>Salah</b><small>ulang 10 menit</small></div><div class="upi-rate hard" data-rating="hard" role="button" tabindex="0"><b>Sulit</b><small>ulang lebih cepat</small></div><div class="upi-rate good" data-rating="good" role="button" tabindex="0"><b>Ingat</b><small>jeda bertambah</small></div><div class="upi-rate easy" data-rating="easy" role="button" tabindex="0"><b>Mudah</b><small>jeda panjang</small></div></div></div></section><section class="upi-review-panel card"><div class="section-title">🧠 Prioritas repetisi</div><p class="upi-review-copy">Kartu yang sering salah atau sudah jatuh tempo akan diprioritaskan.</p><div data-upi-review-list class="upi-review-list"></div></section>`;root.querySelector('[data-upi-material-back]')?.addEventListener('click',()=>show(upiDetailId(block)));const flipEl=root.querySelector('[data-upi-flip]');flipEl?.addEventListener('click',()=>upiToggleFlipMaterial(block,material));flipEl?.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();upiToggleFlipMaterial(block,material);}});root.querySelectorAll('.upi-rate').forEach(el=>{const act=()=>upiRateMaterial(block,material,el.dataset.rating);el.addEventListener('click',act);el.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();act();}});});upiRenderMaterial(block,material,upiPickMaterial(block,material));}
...
function upiRenderMaterial(block,material,card){const root=document.getElementById(upiMaterialPageId(block,material));if(!root)return;const cards=upiMaterialCards(block,material),reviews=upiEnsureMaterialReviews(block,material),current=card||upiPickMaterial(block,material);root._upiCurrent=current;root._upiRevealed=false;const idx=Math.max(1,cards.findIndex(c=>c.id===current?.id)+1),setText=(sel,v)=>{const el=root.querySelector(sel);if(el)el.textContent=v;};setText('[data-upi-index]',current?`Kartu ${idx} / ${cards.length}`:'Belum ada kartu');setText('[data-upi-topic]',current?.topic||material);setText('[data-upi-prompt]',current?.prompt||`Belum ada kartu ${material} untuk blok ${block}.`);setText('[data-upi-answer]',current?.answer||'—');setText('[data-upi-explanation]',current?.explanation||'Tambahkan bank flash card untuk deck ini.');const img=root.querySelector('[data-upi-image]');if(img){img.src=current?.image||'assets/flashcards/heart.svg';img.alt=current?`Ilustrasi ${current.topic}`:'Belum ada kartu';img.style.cursor=current?'zoom-in':'default';img.onclick=e=>{e.stopPropagation();if(current)upiOpenImageZoom(current.image,current.topic||'Gambar flashcard');};}const flip=root.querySelector('[data-upi-flip]');if(flip){flip.classList.remove('is-flipped');flip.setAttribute('aria-pressed','false');flip.toggleAttribute('aria-disabled',!current);flip.setAttribute('aria-label',current?'Kartu flashcard. Ketuk untuk membuka jawaban.':'Belum ada kartu di deck ini.');}root.querySelector('[data-upi-rating]')?.setAttribute('hidden','');const due=cards.filter(c=>upiDue(c,reviews[c.id],Date.now())).length;setText('[data-upi-due]',String(due));}
function upiToggleFlipMaterial(block,material){const root=document.getElementById(upiMaterialPageId(block,material));if(!root||!root._upiCurrent)return;const flip=root.querySelector('[data-upi-flip]');if(!flip)return;const flipped=flip.classList.toggle('is-flipped');flip.setAttribute('aria-pressed',String(flipped));if(flipped)upiRevealMaterial(block,material);}
function upiRevealMaterial(block,material){const root=document.getElementById(upiMaterialPageId(block,material));if(!root||!root._upiCurrent)return;root._upiRevealed=true;root.querySelector('[data-upi-rating]')?.removeAttribute('hidden');}
...
async function authLogout(){
  ...
  authServerUser=null;try{window.CLINED_ONLINE_SYNC?.switchUser?.('');}catch(error){console.warn('CLINED account cleanup skipped',error);}
  // Logout manual harus kembali ke landing page (index.html), bukan tetap di app.html dengan modal login.
