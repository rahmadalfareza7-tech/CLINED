...
async function loadServerBanks(){
  ...
  banksLoadPromise=(async()=>{
    const meta=await fetch('/api/banks',{credentials:'same-origin',cache:'no-store'}).then(r=>{if(!r.ok)throw Error('Daftar bank soal tidak dapat dimuat.');return r.json();});
    ...
    const ids=Object.keys(BANKS).filter(id=>!BANKS[id].loaded);
    await Promise.all(ids.map(async id=>{
      const r=await fetch(`/api/banks/${encodeURIComponent(id)}`,{credentials:'same-origin'});
      if(!r.ok)throw Error(`Bank ${BANKS[id].name} tidak dapat dimuat.`);
      const payload=await r.json();
      const b=payload.bank||{};
      BANKS[id].data=Array.isArray(b.questions)?b.questions:[];
      BANKS[id].count=BANKS[id].data.length;
      BANKS[id].version=Number(b.version||BANKS[id].version||1);
      BANKS[id].loaded=true;
    }));
...
async function startQuiz(){
  ...
  try{await loadServerBanks();}catch{return;}
...
function renderQuestion(resume=false){
  ...
  $("question").innerHTML=`<div>${esc(q.soal)}</div><div class="quiz-share-row"><button type="button" class="chat-share-btn" data-chat-share="current-quiz-question">💬 Kirim ke Chat</button></div>`;
  window.__CLINED_SHARE_QUESTIONS=window.__CLINED_SHARE_QUESTIONS||{};
  window.__CLINED_SHARE_QUESTIONS["current-quiz-question"]={soal:q.soal,opsi:q.opsi,block:q.block||blockForBank(selectedBank)||"UAB",bank:BANKS[selectedBank]?.name||selectedBank};
