...
(() => {
  ...
  function install(user){
    ...
    if(!isAdmin && !isHelper){ removeAdminControls(); return; }
    ...
    if(isAdmin && !document.getElementById('adminLoginsPanel')){
      ...
      const renderAnnouncements=async()=>{const c=logins.querySelector('#annListContainer');if(!c)return;try{const d=await request('/api/admin/announcements');const anns=d.announcements||[];c.innerHTML=anns.length?anns.map(a=>`<div class="admin-announcement-item"><div class="admin-announcement-head"><span class="admin-announcement-priority">${esc((a.priority||'normal').toUpperCase())}</span><b>${esc(a.title)}</b><span class="admin-announcement-status ${a.published?'is-published':'is-draft'}">${a.published?'Dipublikasi':'Draft'}</span></div><p>${esc(a.body)}</p><div class="admin-announcement-actions"><button type="button" class="secondary small-btn" data-ann-toggle="${esc(a.id)}">${a.published?'Jadikan Draft':'Publikasikan'}</button><button type="button" class="secondary small-btn danger" data-ann-del="${esc(a.id)}">Hapus</button></div></div>`).join(''):'<p class="muted">Belum ada pengumuman.</p>';c.querySelectorAll('[data-ann-toggle]').forEach(btn=>btn.addEventListener('click',async()=>{const id=btn.dataset.annToggle;const item=anns.find(a=>String(a.id)===String(id));if(!item)return;btn.disabled=true;try{const d=await request('/api/admin/announcements',{method:'POST',body:JSON.stringify({action:item.published?'draft':'publish',id})});await renderAnnouncements();if(d.published&&typeof window.CLINED_refreshAnnouncements==='function')await window.CLINED_refreshAnnouncements();if(typeof showToast==='function')showToast(d.published?'Pengumuman dipublikasikan.':'Pengumuman dikembalikan ke draft.',false);}catch(e){message(logins,e.message||'Aksi pengumuman gagal.',true);btn.disabled=false;}}));c.querySelectorAll('[data-ann-del]').forEach(btn=>btn.addEventListener('click',async()=>{if(!confirm('Hapus pengumuman ini?'))return;btn.disabled=true;try{const d=await request('/api/admin/announcements',{method:'POST',body:JSON.stringify({action:'delete',id:btn.dataset.annDel})});await renderAnnouncements();if(typeof window.CLINED_refreshAnnouncements==='function')await window.CLINED_refreshAnnouncements();if(typeof showToast==='function')showToast(d.message||'Pengumuman dihapus.',false);}catch(e){message(logins,e.message||'Penghapusan gagal.',true);btn.disabled=false;}}));}catch(e){c.textContent='Gagal memuat: '+(e&&e.message||'error');}};
      logins.querySelector('#saveAnnBtn')?.addEventListener('click',async()=>{const title=logins.querySelector('#annTitleInput')?.value?.trim();const body=logins.querySelector('#annBodyInput')?.value?.trim();const priority=logins.querySelector('#annPrioritySelect')?.value||'normal';if(!title||!body){alert('Judul dan isi harus diisi.');return;}const btn=logins.querySelector('#saveAnnBtn');btn.disabled=true;try{await request('/api/admin/announcements',{method:'POST',body:JSON.stringify({title,body,priority,published:false})});logins.querySelector('#annTitleInput').value='';logins.querySelector('#annBodyInput').value='';await renderAnnouncements();if(typeof showToast==='function')showToast('Pengumuman tersimpan sebagai draft.',false);}catch(e){alert(e.message||'Gagal menyimpan.');}finally{btn.disabled=false;}});
  ...
  async function hydrateMaterials(){try{const d=await request('/api/materials');const map=Object.fromEntries((d.materials||[]).map(x=>[String(x.block).toUpperCase(),x.url]));
    ...
    document.querySelectorAll('[data-materi-section]').forEach(host=>{const block=String(host.dataset.materiSection||'').toUpperCase();const url=map[block];if(url){host.innerHTML=`<a class="materi-button" href="${url.replace(/"/g,'&quot;')}" target="_blank" rel="noopener noreferrer" aria-label="Buka MATERI">MATERI</a>`;}else{host.innerHTML='';}});
