...
(() => {
  ...
  async function hydrateGlobalBanks(){try{
    ...
    window.renderBanks?.();window.renderCounts?.();await syncUabNodeStates();
  ...
  async function boot(){try{const d=await request('/api/auth/me');if(d.user){install(d.user);await Promise.all([hydrateMaterials(),hydrateGlobalBanks()]);}else removeAdminControls();}catch(e){removeAdminControls();}}
