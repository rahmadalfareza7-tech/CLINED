...
(() => {
  ...
  async function sync({force=false} = {}) {
    ...
    const needsPull = force || dirty || queue.length || (Date.now() - lastPullAt >= PERIODIC_PULL_MS);
    if (!needsPull) return;
    sending = true;
    try {
      ...
      if (!user) { dirty = false; lastPullAt = Date.now(); return; }
      ...
      // Keep the queue/dirty flag for the next successful sync.
    } finally {
      sending = false;
    }
  }
  ...
  setInterval(() => sync(), PERIODIC_PULL_MS);
